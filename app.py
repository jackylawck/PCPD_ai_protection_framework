...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 🏛 shrink 全球管治框架對齊" if is_zh else "### 🏛️ Global Frameworks")
    st.caption("✅ **IAPP AIGP (v2.1):** Domain II, III, IV 精準映射")
    st.caption("✅ **ISO/IEC 42001 (AIMS):** 體系整合與可觀測性")
    st.markdown("---")
    st.warning("⚠️ **開源公共利益沙盒：** 純決定性代碼架喚，100% 無 AI 幻覺，數據關閉即銷毀。")
    ...
    with tab1:
        ...
        if base_risk["level"] == "HIGH":
            if "人在環中" not in current_oversight:
                st.warning(f"⚠️ **管治衝突警告：** 依據 **PCPD 2024《模範框架》第二部第 32(i) 條** 明文規定，高風險 AI 系統**應強制採取「人在環中」(Human-in-the-loop)** 方式。人類決策者必須在決策過程中保留控制權，以防止或減低 AI 出錯或不當自動化決定。")
            else:
                st.success("✅ **人為監督配置合規：** 完全符合 **PCPD 2024《模範框架》第 32 條** 高風險用例採取「人在環中」之規範，人類保留最終裁量權。")
